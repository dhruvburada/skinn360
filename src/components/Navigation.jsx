...
const NAV_ITEMS = ['Treatments', 'The Doctor', 'Why Us', 'Reviews'];
...
export const Navigation = () => {
  ...
  const sectionHref = (label) => {
    ...
    return location.pathname === '/' ? `#${slug}` : `/#${slug}`;
  };
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'py-4 bg-white/80 backdrop-blur-xl shadow-sm border-b border-white/20' : 'py-8 bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <button type="button" className="md:hidden text-gray-800" onClick={() => setIsOpen(!isOpen)} aria-label={isOpen ? 'Close menu' : 'Open menu'}>
          {isOpen ? <X /> : <Menu />}
        </button>
      </div>
